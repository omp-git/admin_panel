                    ...
                    <tr>
                            ...
                            @if($table->dataTypeId)
                                <a href="{{ route('voyager.' . $table->slug . '.index') }}"
                                   class="btn-sm btn-warning browse_bread">
                                    ...
                                    <span>{{ __('voyager::database.browse_bread') }}</span>
                                </a>
                                <a href="{{ route('voyager.bread.edit', $table->name) }}"
                                   class="btn-sm btn-default edit">
                                   <i class="voyager-pen"></i>
                                   <i class="voyager-bread"></i>
                                    <span>{{ __('voyager::bread.edit_bread') }}</span>
                                </a>
                                <a data-id="{{ $table->dataTypeId }}" data-name="{{ $table->name }}"
                                     class="btn-sm btn-danger delete"><i class="voyager-trash"></i><i class="voyager-bread"></i><span>{{ __('voyager::bread.delete_bread') }}</span></a>
                            @else
                                <a href="{{ route('voyager.bread.create', ['name' => $table->name]) }}"
                                   class="btn-sm btn-default add-bread">
                                    <i class="voyager-bread"></i>
                                    <i class="voyager-plus bigger-130"></i>
                                    <span>{{ __('voyager::bread.add_bread') }}</span>
                                </a>
                            @endif
                            </div>
                        </td>
                        ...
                        <td class="actions">
                            ...
                            <a href="{{ route('voyager.database.edit', $table->name) }}"
                               class="btn btn-sm btn-primary pull-right edit_table" style="display:inline; margin-right:10px;">
                               <i class="voyager-edit"></i> <span>{{ __('voyager::generic.edit') }}</span>
                            </a>
                            <a href="{{ route('voyager.database.show', $table->name) }}"
                               data-name="{{ $table->name }}"
                               class="btn btn-sm btn-warning pull-right desctable view_table" style="display:inline; margin-right:10px;">
                               <i class="voyager-eye"></i> <span>{{ __('voyager::generic.view') }}</span>
                            </a>
                        </td>
                    </tr>
                @endforeach
            ...
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <form action="{{ route('voyager.bread.delete', ['id' => null]) }}" id="delete_bread_form" method="POST">
                        ...
                        <input type="submit" class="btn btn-danger  pull-right" value="{{ __('voyager::bread.delete_bread_conf') }}">
